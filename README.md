...
stale.
    ...
    src/compass.js       the compass, top-left: a rose that follows the map
                         or your hand, and four letters that stay upright
    src/compass-art.js   GENERATED by tools/compass.py — the rose and N, E,
                         S, W as five CSS masks
    ...
    tools/compass.py     assets/compass.png → src/compass-art.js: the one
                         drawing cut into the rose and its four letters
...
plate's pitch, through `Title.paint` — and the acronym's letters are Roboto Slab read back
as cells through `Title.face`, one diamond per cell of ink, the way the
town's title is set (the row's letters and the words on the diagonal are
the chrome's own mono); and it takes no pointer of its own: only a press that lands on a
