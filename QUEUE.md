...
- [x] **Eight numbered slots a room, and the view down to just them** —
  ...
  / 6 7 8` — eight round the edge, numbered on across the palace as
  `(room − 1) * 8 + i`. Tones, and no new colour: 1 white bone, 2 green
  park, 3 pink flare, 4 blue creek, 5 orange stairs, 6 red rug, 7 yellow
  (gold pulled half way to bone — a quarter read as the amber twice),
  8 black (`dim`, Eden's call, because the ground is #1B1B21). A marker
  dropped in a room takes the nearest free slot and its number; the ninth
  is refused before the blocks are paid; `reorder` moves a locus through
  the building rather than renumbering it; `reseat` carries the loci when a
  room is resized. The view keeps the walls, dims the line to 0.28, drops
  the rings on the fittings, draws every room's eight (the traced room
  whole, the rest faint), and a locus in a slot stops drawing its own
  number while the grid is up. `src/trace.js`, `src/markers.js`. BUILD 238.

- [x] **The assets in the builder, no door, a reset** — 2026-08-29.
  `tools/assets.py import` writes glyphs.js from the folders (327 glyphs
  in 10 sets); kinds.js offers each set as a print kind under Buildings
  (landmark's set renamed `landmarks`); stock prices for them, free on
  home. `DOOR = false`: no login, Player rows hidden. *Reset — blank
  page* under Town: `Snap.load` of an empty town after a confirm.
  Verified on the v8.1 town: no door, ten sets, prints laid from trees,
  plants and mountains, old landmarks intact. BUILD 219.
